...
# bc KULLANMADAN, önce sayısal verileri filtreleyip sparkline üret
spark(){
  # 1) sadece sayısal değerleri al
  local nums=() v
  for v in "$@"; do
    [[ "$v" =~ ^-?[0-9]+([.][0-9]+)?$ ]] && nums+=("$v")
  done
  ...
  # 4) her değeri 0..7 skalasına yerleştir
  local bars=(▁ ▂ ▃ ▄ ▅ ▆ ▇ █) out=""
  for v in "${nums[@]}"; do
...
render_once(){
  ...
  # Eşik/Alarm
  notify_hit(){ command -v notify-send >/dev/null 2>&1 || return 0; local s="$1" price="$2" dir="$3" thr="$4"; notify-send "CAL Ticker" "${s} ${price%.*} ${dir} ${thr%.*}"; }
  chk_thr(){ local s="$1" price="$2"; local up="ALERT_${s}_ABOVE" down="ALERT_${s}_BELOW"; local a="${!up:-}" b="${!down:-}"
    [[ -n "$price" ]] || return 0
  ...
  # Sparkline → sadece sayısal kayıt
  ...
  LABEL_W=5; PRICE_W=8
  {
    ...
    if (( ${SHOW_SPARKLINE:-1} )) && [[ -f "$HIST_FILE" ]]; then
      ...
      # sadece sayısal değerleri gönder
      filtered=()
